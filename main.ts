...
const SSD1309_SETCONTRAST = 0x81
    ...
    const SSD1309_SETPAGEADRESS = 0x22
    const SSD1309_DISPLAYALLON_RESUME = 0xA4
    const SSD1309_DISPLAYALLON = 0xA5
    const SSD1309_NORMALDISPLAY = 0xA6
    const SSD1309_INVERTDISPLAY = 0xA7
    const SSD1309_DISPLAYOFF = 0xAE
    const SSD1309_DISPLAYON = 0xAF
    const SSD1309_SETDISPLAYOFFSET = 0xD3
    const SSD1309_SETCOMPINS = 0xDA
    const SSD1309_SETVCOMDETECT = 0xDB
    const SSD1309_SETDISPLAYCLOCKDIV = 0xD5
    const SSD1309_SETPRECHARGE = 0xD9
    const SSD1309_SETMULTIPLEX = 0xA8
    const SSD1309_SETLOWCOLUMN = 0x00
    const SSD1309_SETHIGHCOLUMN = 0x10
    const SSD1309_SETSTARTLINE = 0x40
    const SSD1309_MEMORYMODE = 0x20
    const SSD1309_COMSCANINC = 0xC0
    const SSD1309_COMSCANDEC = 0xC8
    const SSD1309_SEGREMAP = 0xA0
    const SSD1309_CHARGEPUMP = 0x8D
    ...
    export function clear() {
        loadStarted = false
        loadPercent = 0
        command(SSD1309_SETCOLUMNADRESS)
        command(0x00)
        command(displayWidth - 1)
        command(SSD1309_SETPAGEADRESS)
        command(0x00)
    ...
    export function init(width: number, height: number) {
        command(SSD1309_DISPLAYOFF);
        command(SSD1309_SETDISPLAYCLOCKDIV);
        command(0x80);                                  // the suggested ratio 0x80
        command(SSD1309_SETMULTIPLEX);
        command(0x3F);
        command(SSD1309_SETDISPLAYOFFSET);
        command(0x0);                                   // no offset
        command(SSD1309_SETSTARTLINE | 0x0);            // line #0
        command(SSD1309_CHARGEPUMP);
        command(0x14);
        command(SSD1309_MEMORYMODE);
        command(0x00);                                  // 0x0 act like ks0108
        command(SSD1309_SEGREMAP | 0x1);
        command(SSD1309_COMSCANDEC);
        command(SSD1309_SETCOMPINS);
        command(0x12);
        command(SSD1309_SETCONTRAST);
        command(0xCF);
        command(SSD1309_SETPRECHARGE);
        command(0xF1);
        command(SSD1309_SETVCOMDETECT);
        command(0x40);
        command(SSD1309_DISPLAYALLON_RESUME);
        command(SSD1309_NORMALDISPLAY);
        command(SSD1309_DISPLAYON);
        displayWidth = width
